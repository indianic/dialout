...
exports.setActiveSocket = setActiveSocket;
exports.isPtyAvailable = isPtyAvailable;
exports.openSession = openSession;
exports.clipboardCommand = clipboardCommand;
exports.copyVerb = copyVerb;
exports.clipboardBindings = clipboardBindings;
exports.applyNativeTmuxOptions = applyNativeTmuxOptions;
exports.tmuxSessionExists = tmuxSessionExists;
exports.applyBrowserSessionOptions = applyBrowserSessionOptions;
exports.openAttach = openAttach;
exports.writeToSession = writeToSession;
exports.resizeSession = resizeSession;
exports.closeSession = closeSession;
exports.detachPtySession = detachPtySession;
...
function openSession(id, command, cwd, cols = 80, rows = 24, opts = {}) {
    if (!pty)
        return false;
    ...
    if (opts.coworkWrap) {
        ...
        const tmuxName = (0, tmux_manager_1.tmuxSessionName)(id);
        ...
        try {
            const tmuxRun = (args) => {
                (0, child_process_1.execFileSync)('tmux', args, { timeout: 5000, stdio: 'pipe' });
            };
            ...
            if (tmuxSessionExists(tmuxName)) {
                ...
                let branch = '';
                try {
                    branch = (0, child_process_1.execFileSync)('git', ['-C', resolvedCwd, 'rev-parse', '--abbrev-ref', 'HEAD'], { timeout: 3000, stdio: 'pipe' }).toString().trim();
                }
                catch { }
            ...
            if (createdByUs) {
                try {
                    (0, child_process_1.execFileSync)('tmux', ['kill-session', '-t', tmuxName], { timeout: 5000, stdio: 'pipe' });
                }
                catch { }
            }
            proc = undefined;
...
function clipboardCommand(deps = {}) {
...
function clipboardBindings() {
    ...
    if (command)
        cmds.push(['set-option', '-g', 'copy-command', command]);
    cmds.push(['bind-key', '-T', 'copy-mode', 'MouseDragEnd1Pane', 'send-keys', '-X', ...copy], ['bind-key', '-T', 'copy-mode-vi', 'MouseDragEnd1Pane', 'send-keys', '-X', ...copy], ['bind-key', '-T', 'copy-mode', 'Enter', 'send-keys', '-X', ...copy], ['bind-key', '-T', 'copy-mode-vi', 'y', 'send-keys', '-X', ...copy]);
    return cmds;
}
function applyNativeTmuxOptions() {
    ...
    try {
        const feats = (0, child_process_1.execFileSync)('tmux', ['show-options', '-g', 'terminal-features'], { timeout: 5000, stdio: 'pipe' }).toString();
        if (!/RGB/.test(feats)) {
            (0, child_process_1.execFileSync)('tmux', ['set-option', '-sa', 'terminal-features', '*:RGB'], { timeout: 5000, stdio: 'pipe' });
        }
    }
    catch { /* no server yet */ }
}
function tmuxSessionExists(name, deps = {}) {
    const list = deps.list ?? (() => (0, child_process_1.execFileSync)('tmux', ['list-sessions', '-F', '#{session_name}'], { timeout: 5000, stdio: 'pipe' }).toString());
    try {
...
function applyBrowserSessionOptions(name) {
    try {
        (0, child_process_1.execFileSync)('tmux', ['set-option', '-t', name, 'status', 'off'], { timeout: 5000, stdio: 'pipe' });
    }
    catch { /* session gone or option unsupported — cosmetic, never fatal */ }
}
function openAttach(id, tmuxName, readOnly, cols = 80, rows = 24) {
    if (!pty)
        return false;
    if (sessions.has(id))
        return true; // idempotent reattach
    applyNativeTmuxOptions();
    const args = ['attach-session', '-t', tmuxName];
    if (readOnly)
        args.push('-r', '-f', 'ignore-size'); // Peek: don't drive window size (tmux ≥3.2)
    let proc;
    try {
...
async function closeSession(id) {
    ...
    if (tmux?.creator) {
        ...
        setTimeout(async () => {
            const clients = await (0, tmux_manager_1.countClients)(tmux.name);
            if (clients === 0)
                await (0, tmux_manager_1.killTmuxSession)(tmux.name);
        }, 500);
    }
