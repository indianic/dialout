...
function textOf(value) {
    ...
    if (Array.isArray(value)) {
        return value.map((b) => (typeof b?.text === 'string' ? b.text : '')).join('');
    }
    return '';
}
            ...
            switch (block.type) {
                ...
                case 'tool_result':
                    events.push({
                        ...
                        ok: !block.is_error, preview: (0, types_1.preview)(textOf(block.content)), at,
                    });
                    break;
                case 'thinking':
                ...
                default: {
                    ...
                    if (text)
                        events.push({ kind: 'message', role: rec.type, text, id, at });
                }
            }
        }
        return events;
