...
exports.grokTranscriptPath = grokTranscriptPath;
exports.claudeProjectDir = claudeProjectDir;
exports.defaultProcStartMs = defaultProcStartMs;
exports.pickNewest = pickNewest;
exports.grokTranscript = grokTranscript;
exports.locateTranscript = locateTranscript;
...
function grokTranscriptPath(home, cwd, sessionId) {
...
function claudeProjectDir(configHome, cwd) {
...
function defaultProcStartMs(pid) {
    try {
        const out = (0, child_process_1.execFileSync)('ps', ['-o', 'lstart=', '-p', String(pid)], { timeout: 5000, stdio: 'pipe' }).toString().trim();
...
function defaultWriteHandles(pid) {
    try {
        // -F n prints one 'n<path>' line per open file.
        const out = (0, child_process_1.execFileSync)('lsof', ['-p', String(pid), '-F', 'n'], { timeout: 5000, stdio: 'pipe' }).toString();
...
function defaultProcCwd(pid) {
    try {
        if (process.platform === 'linux')
            return fs.readlinkSync(`/proc/${pid}/cwd`);
        const out = (0, child_process_1.execFileSync)('lsof', ['-a', '-p', String(pid), '-d', 'cwd', '-F', 'n'], { timeout: 5000, stdio: 'pipe' }).toString();
...
function defaultProcEnv(pid) {
    const env = {};
    try {
        const raw = process.platform === 'linux'
            ? fs.readFileSync(`/proc/${pid}/environ`, 'utf8').split('\0')
            : (0, child_process_1.execFileSync)('ps', ['eww', '-p', String(pid), '-o', 'command='], { timeout: 5000, stdio: 'pipe' }).toString().split(' ');
...
// every poll.
function defaultTranscriptCwds(path) {
    try {
        ...
        for (const line of buf.subarray(0, read).toString().split('\n')) {
            ...
            try {
                ...
                if (typeof rec.cwd === 'string')
                    found.add(rec.cwd);
                else if (typeof rec?.payload?.cwd === 'string')
                    found.add(rec.payload.cwd);
            }
...
function defaultGrokSessions(home) {
    try {
...
function defaultGrokNewest(cwdDir) {
    try {
...
function locateTranscript(pid, kind, deps = {}) {
    ...
    if (recorded.length && !recorded.includes(cwd))
        return null;
    return candidate;
}
//# sourceMappingURL=ai-transcript-locator.js.map