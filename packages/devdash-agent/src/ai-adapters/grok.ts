...
export const grokAdapter: AiAdapter = {
    ...
    switch (rec.type) {
      ...
      case 'assistant': {
        ...
        for (const call of Array.isArray(rec.tool_calls) ? rec.tool_calls : []) {
          ...
          if (typeof call.arguments === 'string') {
            try { input = JSON.parse(call.arguments); } catch { /* keep the raw string */ }
          }
          out.push({
            kind: 'tool_call',
            name: String(call.name || 'tool'),
            summary: preview(typeof call.arguments === 'string'
              ? call.arguments
      ...
      case 'tool_result': {
        return [{
          ...
          ok: true,
          preview: preview(textOf(rec.content)),
          at: NO_TIMESTAMP,
        }];
      }
  ...
  title(record: unknown): string | null {
    ...
    if (rec.type !== 'user' || !isRealPrompt(rec)) return null;
    if (rec.prompt_index !== 0) return null; // only the FIRST prompt names the session
