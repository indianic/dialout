  ...
  .action(async () => {
    ...
    const remoteDefault =
      savedRemote && !/^wss?:\/\/(localhost|127\.0\.0\.1)([:/]|$)/.test(savedRemote)
        ? savedRemote
        : DEFAULT_SERVER_URL;
    const remoteIsSaved = remoteDefault !== DEFAULT_SERVER_URL;

    console.log('\x1b[1mWhere should this agent connect?\x1b[0m');
    console.log('');
    console.log(
      `  \x1b[36m1)\x1b[0m Remote server — ${remoteDefault}` +
        (remoteIsSaved ? ' \x1b[90m(saved)\x1b[0m' : ''),
    );
    if (remoteIsSaved) {
      console.log(`     \x1b[90mthis build ships with ${DEFAULT_SERVER_URL} — pick 3 to switch\x1b[0m`);
    }
 ...
 */
async function chooseServiceScope(): Promise<boolean> {
  if (process.platform === 'linux' && defaultLinuxScope() === 'system') {
