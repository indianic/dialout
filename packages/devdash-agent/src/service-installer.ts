...
export function serviceUser(env: NodeJS.ProcessEnv = process.env, info: () => string = () => os.userInfo().username): string {
...
function canPromptSudo(): boolean {
...
function runPrivileged(cmd: string, deps: UninstallDeps = {}): boolean {
  const root = deps.isRoot ?? isRoot;
  const prompt = deps.canPromptSudo ?? canPromptSudo;
  const run = deps.run ?? ((c: string, o?: object) => { execSync(c, o as never); });
  ...
  if (prompt()) {
    ...
    try { run(`sudo bash -c '${cmd}'`, { stdio: 'inherit' }); return true; } catch { return false; }
  }
  return false;
}
...
function removeLaunchAgent(): void {
...
function installMacOSAgent(): void {
  const plistPath = getLaunchdPlistPath();
...
function installMacOSDaemon(): void {
  const target = getLaunchDaemonPlistPath();
  const staging = path.join(configDirFor(os.homedir()), `${SERVICE_NAME}.plist`);
  ...
  if (isRoot()) {
    ...
    execSync(`chown root:wheel ${target}`);
    execSync(`chmod 644 ${target}`);
    try { execSync(`launchctl bootout system ${target} 2>/dev/null`); } catch { /* not loaded */ }
    try {
      execSync(`launchctl bootstrap system ${target}`);
    } catch {
      execSync(`launchctl load ${target}`); // older macOS fallback
    }
    ...
    try {
      execSync(
        `sudo bash -c 'cp "${staging}" "${target}" && chown root:wheel "${target}" && chmod 644 "${target}" && { launchctl bootout system "${target}" 2>/dev/null; launchctl bootstrap system "${target}"; }'`,
        { stdio: 'inherit' }
      );
...
function printMacDaemonSudoSteps(staging: string, target: string): void {
  console.log('');
...
function uninstallMacOS(deps: UninstallDeps = {}): UninstallResult {
  ...
  if (!removed && pending.length === 0) log('Service not installed.');
  return { removed, pending };
}

// Returns the running PID of a launchd job, or null if loaded-but-stopped / absent.
function launchdPid(domainTarget: string): number | null {
  try {
    const out = execSync(`launchctl print ${domainTarget} 2>/dev/null`, { encoding: 'utf-8' });
...
function getSystemdUnitPath(homedir: string = os.homedir()): string {
  return path.join(homedir, '.config', 'systemd', 'user', `${SYSTEMD_UNIT}.service`);
}

function getSystemdSystemUnitPath(): string {
  return `/etc/systemd/system/${SYSTEMD_UNIT}.service`;
}

function generateSystemdUnit(system: boolean): string {
  const nodePath = getNodePath();
  const script = getAgentScript();
...
// A systemd USER unit lives inside user@<uid>.service, and systemd-logind
// stops that manager when the user's LAST session ends — taking every unit
// inside it down with it. On a desktop that is invisible: the graphical
// session is the login. On a headless box reached only over SSH it means the
// agent dies every single time you close the terminal, which is precisely
// when it needs to be running.
//
// `loginctl enable-linger` is systemd's own answer: it keeps the user manager
// alive with no session present, and starts it at boot. A user unit without it
// is not a service, it is a session process with extra steps.
...
export function isLingerEnabled(user: string): boolean {
  try {
    return /Linger=yes/i.test(execSync(`loginctl show-user ${user}`, { encoding: 'utf-8', stdio: 'pipe' }));
  } catch {
    return false; // no such user record yet — enable-linger creates one
  }
}

export function enableLinger(user: string, deps: LingerDeps = {}): LingerResult {
  const isEnabled = deps.isEnabled ?? isLingerEnabled;
  const run = deps.run ?? ((cmd: string) => { execSync(cmd, { stdio: 'pipe' }); });

  if (isEnabled(user)) return { ok: true, alreadyOn: true };
  try {
...
// Which scope to install when the caller passed neither --system nor --login.
//
// As root the system unit is strictly better and costs nothing: no sudo
// prompt to answer, starts at boot rather than at login, and lives in
// system.slice where no logout can reach it. The old code defaulted to the
// user unit here and asked "start at boot? Requires admin password. [y/N]" —
// a question whose stated cost is false for root and whose default answer
// installs the one variant that dies on logout.
export interface LinuxScopeDeps {
  isRoot?: () => boolean;
}

export function defaultLinuxScope(deps: LinuxScopeDeps = {}): 'system' | 'user' {
  const root = deps.isRoot ?? isRoot;
  return root() ? 'system' : 'user';
}

function installLinuxUser(): void {
...
function installLinuxSystem(): void {
  const target = getSystemdSystemUnitPath();
  const staging = path.join(configDirFor(os.homedir()), `${SYSTEMD_UNIT}.service`);
  fs.writeFileSync(staging, generateSystemdUnit(true));

  if (isRoot()) {
    fs.copyFileSync(staging, target);
    execSync('systemctl daemon-reload');
    execSync(`systemctl enable ${SYSTEMD_UNIT}`);
    execSync(`systemctl restart ${SYSTEMD_UNIT}`);
    console.log(`System service installed: ${target}`);
    console.log('The agent will start at boot, before login.');
  } else if (canPromptSudo()) {
    console.log('Boot service requires admin rights — you may be prompted for your password.\n');
    try {
      execSync(
        `sudo bash -c 'cp "${staging}" "${target}" && systemctl daemon-reload && systemctl enable --now ${SYSTEMD_UNIT}'`,
        { stdio: 'inherit' }
      );
      console.log(`\nSystem service installed: ${target}`);
      console.log(`The agent will start at boot, before login (as user "${serviceUser()}").`);
...
function printLinuxSystemSudoSteps(staging: string, target: string): void {
  console.log('');
  console.log(`  sudo cp ${staging} ${target}`);
...
function uninstallLinux(deps: UninstallDeps = {}): UninstallResult {
  const exists = deps.existsSync ?? fs.existsSync;
  const log = deps.log ?? console.log;
  let removed = false;
  const pending: string[] = [];
  ...
  if (exists(userPath)) {
    try {
      execSync(`systemctl --user disable --now ${SYSTEMD_UNIT}`);
...
function systemdActive(scope: 'user' | 'system'): { active: boolean; pid: number | null } {
  const flag = scope === 'user' ? '--user ' : '';
  let active = false;
  let pid: number | null = null;
  try {
    active = execSync(`systemctl ${flag}is-active ${SYSTEMD_UNIT} 2>/dev/null`, { encoding: 'utf-8' }).trim() === 'active';
  } catch { /* inactive/failed */ }
  try {
    const mp = parseInt(
      execSync(`systemctl ${flag}show -p MainPID --value ${SYSTEMD_UNIT} 2>/dev/null`, { encoding: 'utf-8' }).trim(),
      10
    );
    pid = mp > 0 ? mp : null;
  } catch { /* none */ }
...
export interface ServiceStatus {
  ...
  atBoot: boolean; // true = starts at boot (before login); false = starts at login
}

export interface InstallOptions {
  system?: boolean; // true = boot-before-login (LaunchDaemon / systemd system unit)
}

export function installService(opts: InstallOptions = {}): void {
...
export function uninstallService(deps: UninstallDeps = {}): UninstallResult {
...
function getWatchdogPath(homedir: string = os.homedir()): string {
  return path.join(configDirFor(homedir), 'watchdog.sh');
}

export function getWatchdogScript(): string {
  const nodePath = getNodePath();
  const script = getAgentScript();
  const pidFile = path.join(configDirFor(os.homedir()), 'daemon.pid');
  const logDir = path.join(configDirFor(os.homedir()), 'logs');
...
export function installCron(intervalMinutes: number = 5): void {
...
export function uninstallCron(): void {
  ...
  if (cleaned.length === 0) {
    execSync('crontab -r 2>/dev/null', { encoding: 'utf-8' }).toString();
  } else {
    cleaned.push('');
  ...
  const watchdogPath = getWatchdogPath();
  if (fs.existsSync(watchdogPath)) fs.unlinkSync(watchdogPath);
...
function parseScriptFromUnit(content: string): string | null {
...
function parseScriptFromPlist(content: string): string | null {
...
function parseScriptFromWatchdog(content: string): string | null {
...
function normalizeScriptPath(p: string, realpath: (p: string) => string = fs.realpathSync): string {
  try {
    return realpath(p);
  } catch {
...
export interface ListSupervisorsDeps {
  ...
  systemdActive?: (scope: 'user' | 'system') => { active: boolean; pid: number | null };
...
export function listSupervisors(deps: ListSupervisorsDeps = {}): SupervisorInfo[] {
  const platform = deps.platform ?? os.platform();
  ...
  const isStale = (targetScript: string | null): boolean =>
    targetScript !== null && normalizeScriptPath(targetScript, realpath) !== normalizedAgentScript;

  const results: SupervisorInfo[] = [];
...
// --- repairWatchdog: fix the one thing that's safe to fix automatically ---
//
// The production incident this closes: a watchdog written by an older
// version of setup-cron (back when the package was named
// an earlier package name) never gets its SCRIPT= line updated after a
// package rename. Every 5 minutes it resurrects the deprecated
// package, which fights the real daemon for the same machine registration.
//
// getWatchdogScript() already knows how to produce correct content — it's
// what setup-cron calls. Repair reuses that exact function (not a
// hand-rolled copy) so there is ONE source of truth for the watchdog body.
// This function touches ONLY that one file: it never deletes a unit, kills a
// process, or edits the crontab (see Constraint 3 in the plan) — a
// coexisting second supervisor is reported by listSupervisors(), not acted
// on here.
 ...
 */
function nextBackupPath(watchdogPath: string): string {
...
export function staleSupervisorAdvice(s: SupervisorInfo): string[] {
  if (s.kind === 'cron') {
    return [
 ...
 */
export function repairWatchdog(): WatchdogRepairResult {
  const watchdogPath = getWatchdogPath();
  ...
  return { repaired: true, from: currentTarget as string, to: agentScript };
}
